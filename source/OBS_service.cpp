...
void OBS_service::createAudioEncoder(void)
{
	...
	if (strcmp(audio_codec, "aac") != 0) {
		...
		audioEncoder = obs_audio_encoder_create(id,
				"audio", nullptr, 0, nullptr);
		// obs_encoder_release(aencoder);
    } else {
        obs_data_t *aacSettings = obs_encoder_defaults("mf_aac");
        audioEncoder = obs_audio_encoder_create("mf_aac", "simple_audio", nullptr, 0, nullptr);
    }

    // Get available encoders
    // obs_data_t *aacSettings = obs_encoder_defaults("mf_aac");

    // audioEncoder = obs_audio_encoder_create("mf_aac", "simple_audio", aacSettings, 0, nullptr);
}
